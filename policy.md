...
## 6. Autonomy thresholds (the dilemma — **tuned & justified**)

These govern what the **agent may approve with NO human involvement**. The defaults were
deliberately conservative; we chose tiered ceilings with a higher confidence bar.
Full justification in [`docs/PRODUCT-DILEMMA.md`](docs/PRODUCT-DILEMMA.md).

| key | value | meaning |
|---|---|---|
| `AUTONOMY-CEILING` Tier 1 | **$750** | Applies to **meals, travel, hardware**. Agent may auto-approve when the USD amount is **≤ $750** for these categories. Above → human. |
| `AUTONOMY-CEILING` Tier 2 | **$350** | Applies to **saas, other**. Agent may auto-approve when the USD amount is **≤ $350** for these categories. Above → human. |
| `AUTONOMY-CONFIDENCE` | **0.85** | The agent may auto-approve only when its `confidence` is **≥ 0.85**. Below → human. |
...
**An item is `auto_approve` only if ALL hold:** amount ≤ `AUTONOMY-CEILING` (tier-appropriate) · `confidence` ≥ `AUTONOMY-CONFIDENCE` ·
